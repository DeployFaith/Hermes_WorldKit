...
func _update_underwater_visuals() -> void:
	...
	# Debug: print when underwater or near waterline
	if underwater or depth > 0.0 or camera.global_position.y < 1.0:
		print("[Player] cam_y=%.2f depth=%.2f underwater=%s mesh_visible=%s" % [camera.global_position.y, depth, underwater, _water_mesh.visible if _water_mesh else "N/A"])
	# Delay mesh hiding by 2 frames so the underwater environment/fog renders first
	if underwater:
